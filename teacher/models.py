...
class Assignment(models.Model):
    ...
    def __str__(self):
        ...
        teacher_name = self.teacher.user.first_name if self.teacher else "No teacher"

        start = self.start_time.date() if self.start_time else "No start"
        due   = self.due_time.date()   if self.due_time   else "No due"

        return f"{subject_name} - {teacher_name} ({start} → {due})"
...
class Grade(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    student_assignment = models.ForeignKey(Assignment, on_delete=models.CASCADE,related_name='grade')
